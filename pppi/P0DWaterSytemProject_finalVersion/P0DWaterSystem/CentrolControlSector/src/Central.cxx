...
void ReadDepth(std::shared_ptr<asio::io_service> ptr_io, std::string fileName, int* lowestBag, int* lowestDepth)
{
	...
	int linecounter = 0;
	int tmpbag = -1, tmpdepth = DEPTHMAXCUT;//which is 1960 for now;//in mm. btw 77in and 78in
	{
	...
	else
	{
		...
		if(tmpbag==-1&&tmpdepth==DEPTHMAXCUT)//all bags are above DEPTHCUT;
		{
			SendMsg("AllDone50");	
			{
