...
//	std::cout<<"cmdId: "<<cmdId<<" bagId: "<<bagId<<std::endl;
    ...
    switch(cmdId)
    {
        case COMMAND4PPSTART: //fill
			...
			{
					std::lock_guard<std::mutex> lock(lock_PPOperation);
					addr = BoardChannelTodo.board;
					relay = BoardChannelTodo.channel;
					if(CheckRelayState(addr, relay))
					{
							std::cout<<"bag "<<bagId<<" whose Relay "<<addr<<" "<<relay<<" is still Open although it's asked to be closed"<<std::endl;
							std::cout<<"It's going to be asked to close again. But PAY ATTENTION TO THIS BAG!!!"<<std::endl;
							piplate_relay_off(addr,relay);
					}
			}
